...
CUploadQueue::CUploadQueue()
{
	...
	internetmaybedown=false; //Xman x4
...
bool CUploadQueue::AcceptNewClient(bool addOnNextConnect)
{
	...
	uint16 MaxSlots=max((uint16)ceil(MinSlots*1.33),(uint16)ceil(thePrefs.m_slotspeed) + MinSlots);
	...
	if(curUpSlots>=MaxSlots)
	{
		...
		if(theApp.uploadBandwidthThrottler->needslot==true && checkforuploadblock)
		{
			...
			for(POSITION pos=uploadinglist.GetHeadPosition();pos!=NULL;)
			{
				...
				{
					if(cur_client->GetFileUploadSocket()->blockedsendcount > maxblock)
					{
						maxblock=cur_client->GetFileUploadSocket()->blockedsendcount;
			...
			{
				RemoveFromUploadQueue(blockclient,_T("client is blocking too often")); //remark: uploadstopreason: other
...
// <== {Webcache} [Max] 
		...
		else if ( client->Compare(cur_client) ) 
		{
			...
			if (cur_client->credits != NULL && cur_client->credits->GetCurrentIdentState(cur_client->GetIP()) == IS_IDENTIFIED)
			{
				...
				if (thePrefs.GetVerbose())
					AddLeecherLogLine(false,CString(GetResString(IDS_SAMEUSERHASH)),client->GetUserName(),cur_client->GetUserName(),client->GetUserName() );
				return;
			}
			if (client->credits != NULL && client->credits->GetCurrentIdentState(client->GetIP()) == IS_IDENTIFIED)
			{
				...
				if (thePrefs.GetVerbose())
					AddLeecherLogLine(false,CString(GetResString(IDS_SAMEUSERHASH)),client->GetUserName(),cur_client->GetUserName(),cur_client->GetUserName() );
			...
			else
			{
				...
				if (thePrefs.GetVerbose())
					AddLeecherLogLine(false,CString(GetResString(IDS_SAMEUSERHASH)),client->GetUserName(),cur_client->GetUserName(),_T("Both") );
				// ==> SUQWT [Moonlight/EastShare/ MorphXT] - Stulle
				waitinglist.GetAt(pos2)->ClearWaitStartTime(); 
				// <== SUQWT [Moonlight/EastShare/ MorphXT] - Stulle
				RemoveFromWaitingQueue(pos2,true);	
    ...
    if ((uint32)waitinglist.GetCount() >= hardQueueLimit ||
        ...
        client->GetCombinedFilePrioAndCredit() < GetAverageCombinedFilePrioAndCredit()) { // and client has lower credits/wants lower prio file than average client in queue
			...
			uint16 mincontingent=(thePrefs.GetQueueSize()/theApp.sharedfiles->GetCount()/2);
...
#endif
			//Xman Xtreme Upload
			//after reinserting set ready
			//cur_client->GetFileUploadSocket(true)->isready=true;
			//cur_client->socket->isready=true;
